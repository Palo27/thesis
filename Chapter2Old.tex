...
Now we add trading to the dynamics. If the investor buys the stock the quantity 
...
\nu_-(x)=\frac{c_{-}}{1-c_{-} x}.
\end{equation}
%Overall dynamics of the value of the portfolio is then given by
Denote
...
\begin{equation}
%\Delta\ln V_{\tau_k}=-|\Delta\ln(1+c\,G_{\tau_k})|.
\Delta\ln V_{\tau_k}=\ln \frac{V_{\tau_k}}{V_{\tau_k-}}=\ln \frac{V_{\tau_k-}-c\,S^1_{\tau_k}|\Delta N^1_{\tau_k}|}{V_{\tau_k-}}=\ln(1-c\,|\Delta G_{\tau_k}|).
\end{equation}
The total dynamics can be expressed as follows,
\begin{equation}
\label{lnV}
%\ln V_{\tau_k}=[r+(\mu-r)\,G_t-\tfrac{1}{2}\sigma^2\,G_t^2]\,\mathrm{d}t+\sigma\,G_t\,\mathrm{d}W_t-\sum_{k=0}^{\infty}\mathbb{I}_{[\tau_k\leq t]} |\Delta\ln(1+c\,G_{\tau_k})| 
\ln V_{t}=V_0+\int_0^t[r+(\mu-r)\,G_s-\tfrac{1}{2}\sigma^2\,G_s^2]\,\mathrm{d}s+\int_0^t \sigma\,G_s\,\mathrm{d}W_s-\sum_{k=0}^{\infty}\mathbb{I}_{[\tau_k\leq t]} |\Delta\ln(1+c\,G_{\tau_k})|.
\end{equation}
\end{comment}
%Having 
We managed to derive dynamics of $\ln V_{t}$ in terms of $G_{t}$. Now we would like to compute dynamics $G_t$. Since dynamics caused by trading is described by differentials $\mathrm{d}^{\pm} G_t$, we only need to derive the dynamics in case of no trading. Using Ito's formula we gain
\begin{equation*}
...
Dostal \cite{Dostal} derived analytical solution for the problem of maximizing \eqref{criteria}. He showed that
...
\begin{equation*}
P_{+}(g)=\frac{\delta\,\widetilde{b}(g)\Delta t+s^2(g)\Delta t}{2\delta^2},%\geq0
\end{equation*}
...
%\begin{comment}
  ...
  In case of buying or selling, we left the small probability $\epsilon>0$ that the the decision is not realized, in   order to keep the chain irreducible. So if the decision to buy is realized the chain $\widehat{G}$ is shifted up. So the transition probabilities the transition probabilities looks like
  \begin{align*}
    ...
    \begin{center}
        ...
        \subtable[$\mu=0.9$] %
            { \footnotesize
              ...
              %\hline
                   &  $g$  & interval \\ \hline
               Cont. model & $0.19351$ & $(0.350, 0.550)$\\ 
               Discr. appr. & $0.19349$ & $(0.355, 0.550)$\\
               Cont. appr. & ?? & ???\\  %\hline
...
\begin{figure}[h]
\centering
\subfigure[Discrete time approximation]{
   \includegraphics[scale =0.56] {DiscrContour.jpg}
   \label{DiscrContour}
 }
 \subfigure[Continuous time approximation]{
   \includegraphics[scale =0.56] {ContContour.jpg}
   \label{ContContour}
 }
\label{Contour}
\caption{The comparison between discrete time approximation and continuous time approximation. In both cases the parameters are $\mu=0.17$, $c=0.01$, $r=0.025$, $\sigma=0.5$ and $\gamma=1$)}
\end{figure}
\end{comment}
...
\label{Devel}
\caption{Discrete time approximation, process of finding the optimal streategy} %the parameters are $\mu=17$, $c=0.01$, $r=0.025$, $\sigma=0.5$ and $\gamma=1$.}
\end{figure}
